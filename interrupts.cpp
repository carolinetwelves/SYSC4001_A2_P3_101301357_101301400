...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        else if(activity == "FORK")
        {
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////
            //Add your FORK output here - time of thing from trace file
            execution += std::to_string(current_time)+ ", " + std::to_string(duration_intr) + ", cloning the PCB\n";
            current_time += duration_intr;

            //these next two happen after the code excecutes below, this is being put here beacuse it is in the output section outlined

            //call scheduler (0 time)
            execution += std::to_string(current_time) + ", 0, scheduler called\n";
            
            //Call IRET (return from it)
            execution += std::to_string(current_time) + ", 1, IRET\n";
            current_time += 1;
            ///////////////////////////////////////////////////////////////////////////////////////////
            ...
            //push parent to end of wait queue
            wait_queue.push_back(current); 

            //excecute child recursively
            auto [execution_c, system_status_c, current_time_c] = simulate_trace(child_trace, 
                                            current_time,
                                            vectors,
                                            delays,
                                            external_files,
            ...
            //add on/store values from child run time
            execution += execution_c;
            system_status += system_status_c;
            current_time  = current_time_c;

            free_memory(&child); //free the memory of the child (terminates)

            //make parent run
            wait_queue.pop_back();

            //execute scheduler and ISR
        ...
        else if(activity == "EXEC") 
        {
            ...
            int size_program = get_size(program_name, external_files);
            
            execution +=  std::to_string(current_time) + ", " + std::to_string(duration_intr) + ", program is " + std::to_string(size_program) + "Mb large\n";
            current_time += duration_intr;

            //load program (size * 15ms)
            execution +=  std::to_string(current_time) + ", " + std::to_string(size_program*15) + ", loading program into memory\n";
            current_time += size_program * 15;
            ...
            //go through line by line: simulate the trace
            auto [execution_exec, system_status_exec, current_time_exec] = simulate_trace(exec_traces, 
                                            current_time, 
                                            vectors, 
                                            delays,
                                            external_files, 
            ...
            system_status += system_status_exec;
            current_time = current_time_exec;
...
int main(int argc, char** argv) {
    ...
    //external_files is a C++ std::vector of the struct 'external_file'. Check the struct in interrupt.hpp to know more.
    ...
    if(!allocate_memory(&current)) {//allocate memory - .hpp
    ...
    while(std::getline(input_file, trace)) {
        trace_file.push_back(trace); //add new elem to end
    }
